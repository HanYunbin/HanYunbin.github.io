...
const OtherScreen = ({ onBack, userId, db }) => {
  const [activeTab, setActiveTab] = useState('statistics'); // 'statistics', 'ranking', 'diary'

  const renderSubScreen = () => {
    switch (activeTab) {
      case 'statistics':
        return <StatisticsSection onBack={() => setActiveTab('main')} />; // 'main'은 OtherScreen의 기본 탭으로 간주
      case 'ranking':
        return <RankingSection onBack={() => setActiveTab('main')} />;
      case 'diary':
        return <DiarySection userId={userId} db={db} onBack={() => setActiveTab('main')} />;
      default:
        return <StatisticsSection onBack={() => setActiveTab('main')} />;
    }
  };
      ...
      <div className="bg-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700 max-w-4xl w-full text-center mt-8">
        ...
        <div className="flex justify-center mb-6 border-b border-gray-700">
          <button
            ...
            className={`px-6 py-3 text-lg font-semibold rounded-t-lg transition duration-200 ${activeTab === 'statistics' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`}
          >
            통계
          </button>
          <button
            onClick={() => setActiveTab('ranking')}
            className={`px-6 py-3 text-lg font-semibold rounded-t-lg transition duration-200 ${activeTab === 'ranking' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`}
          >
            랭킹
          </button>
          <button
            onClick={() => setActiveTab('diary')}
            className={`px-6 py-3 text-lg font-semibold rounded-t-lg transition duration-200 ${activeTab === 'diary' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-700'}`}
          >
            다이어리
          </button>
        </div>
